...
class EditProfileForm(forms.ModelForm):
	...
	height = forms.IntegerField(widget=forms.NumberInput(), required=False)
	weight = forms.IntegerField(widget=forms.NumberInput(), required=False)
	url = forms.URLField(widget=forms.TextInput(), max_length=60, required=False)

	class Meta:
		model = Profile
		fields = ('picture', 'height', 'weight', 'url')